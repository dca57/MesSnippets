...
const IAToolExampleModal: React.FC<IAToolExampleModalProps> = ({ isOpen, onClose, categoryId, categoryName }) => {
  const { user } = useAuth();
  ...
  const { generateSuggestions, estimateTokensForQuantity, suggestions, setSuggestions, generatedSummary, loading, error, lastUsage } = useIAToolExample();
  ...
  const activeModel = providers.find(p => p.is_active)?.model_id || 'gpt-3.5-turbo';
  ...
  useEffect(() => {
    const estimation = estimateTokensForQuantity(categoryName, [], quantity, description);
    setEstimatedTokens(estimation.totalEstimated);
  }, [description, quantity, categoryName, estimateTokensForQuantity]);

  const remainingTokens = planInfo.limits.maxTokensLLM - planInfo.current.tokens;
  const hasInsufficientTokens = remainingTokens < estimatedTokens;

  const handleGenerate = async () => {
    if (!activeProviderId) {
      setErrorMessage("Aucun modèle IA actif n'a été trouvé. Veuillez en configurer un dans l'administration.");
      return;
    }
    if (!canUse) {
          ...
          <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                Trouvez des pépites pour la catégorie <span className="font-medium text-slate-700 dark:text-slate-300">"{categoryName}"</span>
              </p>
            </div>
            <button 
              onClick={onClose}
...
{/* Token Usage Section */}
            <TokenUsageIndicator 
              className="mb-6 mx-1 px-4 py-3 w-60 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-700 shadow-sm"
            />
